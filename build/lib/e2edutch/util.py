...
def create_example(text, doc_key='example'):
    ...
    for s in sentences:
        # print(s)
        # print(len(s))
        if len(s) > 100 and len(s)<=200:
            s1 = [w for w in s[:100]]
            s2 = [w for w in s[100:]]
            sentences_clipped.append(s1)
            sentences_clipped.append(s2)
        elif len(s) > 200 and len(s)<=300:
            s1 = [w for w in s[:100]]
            s2 = [w for w in s[100:200]]
            s3 = [w for w in s[200:]]
            sentences_clipped.append(s1)
            sentences_clipped.append(s2)
            sentences_clipped.append(s3)
        elif len(s) > 300:
            s1 = [w for w in s[:100]]
            s2 = [w for w in s[100:200]]
            s3 = [w for w in s[200:300]]
            s4 = [w for w in s[200:]]
            sentences_clipped.append(s1)
            sentences_clipped.append(s2)
            sentences_clipped.append(s3)
            sentences_clipped.append(s4)
        else:
            sentences_clipped.append(s)
        
    return {
        "doc_key": doc_key,
        "clusters": [],
        "sentences": sentences_clipped
    }
